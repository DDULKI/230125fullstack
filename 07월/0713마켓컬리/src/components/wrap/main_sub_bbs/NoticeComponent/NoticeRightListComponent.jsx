                                    ...
                                    return(
                                            ...
                                            <dd key={item.번호} data-key={item.번호}>
                                                ...
                                                {/* 
                                                    ...
                                                    1. 파라미터를 링크 태그에 직접 설정. 키=키값& 키=키값  연결.. 
                                                    2. 사용시 선언 import {useSearchParams} from 'react-router-dom';
                                                    3. const [param, setParam] = useSearchParams();
                                                ...
                                                */}
                                                 ...
                                                 {/* 디비 작성을 안하고 서치파람으로 받아야함 */} {/* 쿼리스트링 파라미터 => 글보기 페이지 */}
                                                 <span><Link to={`/notice_view?번호=${item.번호}&제목=${item.제목}&내용=${item.내용}&작성자=${item.작성자}&작성일=${item.작성일}`}>{item.제목} </Link></span>
                                               ...
                                               {/*  
                                                    ...
                                                    3. 훅 선언하고 변수 사용 
                                                       ...
                                                       const Location = useLocation(); 훅 
                                                    4. 변수 속성 이용 후 바인딩
                                                       location.state.아이디   바인딩 
                                                */}
                                                <span>{item.조회수}</span>
                                                <span>{item.작성일}</span>
                                            </dd>
