...
export default function SignUpComponent({회원가입, timer, timerCounterfn, mapAddressFn, comfirmModalOpen}) {
    ...
    React.useEffect(()=>{
        ...
        isEnd && comfirmModalOpen(msg); // isEnd 트루일때 실행하고 아니면 멈춰라 
    ...
    const onClickUserIdOk=(e)=>{
        ...
        if( regExp2.test(value)===false  || regExp3.test(value)===true || regExp4.test(value)===false  ||  regExp5.test(value)===true ){            
            comfirmModalOpen('6자 이상 16자 이하의 영문 혹은 영문과 숫자를 조합');
        }
        else {
            // 아이디 중복검사 실시
            ...
            .then((res)=>{
                ...
                if( result.includes(true) ){                    
                    ...
                    comfirmModalOpen('사용 불가능한 아이디 입니다.');
                }
                else{
                    setState({
                        ...state,
                    ...
                    comfirmModalOpen('사용 가능한 아이디 입니다.');
                }
            })
            .catch((err)=>{
    ...
    const onClickUserEmailOk=(e)=>{
        ...
        if(value===''){
            ...
            comfirmModalOpen('이메일을 입력해 주세요.');      
        }
        else if(regExp.test(value)===false){
     
            comfirmModalOpen('이메일 형식으로 입력해 주세요.');    
        }
        else{
            // 정상인 이메일인 경우
            // 이메일 중복검사
            ...
            .then((res)=>{ //Success
                ...
                try{                    
                    ...
                    if(result.includes(true)){
                        ...
                        comfirmModalOpen('사용 불가능한 이메일 입니다.');    
                    }
                    else{
                        setState({
                            ...state,
                        ...
                        comfirmModalOpen('사용 가능한 이메일 입니다.');  
                    }
                }
                catch(err){
    ...
    const onClickHpNum=(e)=>{
        ...
        if( regExp.test(state.휴대폰)===false ){
            comfirmModalOpen('잘못된 휴대폰 번호 입니다. 확인 후 다시 시도 해 주세요.');  
        }
        else {
            // 발송인증번호 전송 타이밍
            ...
            // 상태관리 변수에 발송인증번호 저장
            발송인증번호 = num;
            comfirmModalOpen(`발송인증번호가 발송되었습니다.  ${num}`);  
        }
        setState({
            ...state,
            발송인증번호: 발송인증번호
    ...
    const onSubmitSignupEvent=(e)=>{
        ...
        if(state.아이디===''){
            ...
            comfirmModalOpen('아이디를 입력하세요');  
        }
        else if(state.아이디중복확인===false){
            comfirmModalOpen('아이디 중복확인을 해주세요');  
      
        }
        else if(state.비밀번호===''){
            comfirmModalOpen('비밀번호를 입력하세요');  
     
        }
        else if(state.비밀번호확인===''){
    
            comfirmModalOpen('한번더 비밀번호를 입력하세요');  
        }
        else if(state.이름===''){

            comfirmModalOpen('이름을 입력하세요');  
        }
        else if(state.이메일===''){
      
            comfirmModalOpen('이메일을 입력하세요');  
        }
        else if(state.이메일중복확인===false){

            comfirmModalOpen( '이메일 중복확인을 해세요');  
        }
        else if(state.휴대폰===''){
           
            comfirmModalOpen( '휴대폰을 입력하세요'); 
        }
        else if(state.isHpOk===false){

            comfirmModalOpen('휴대폰 인증을 해주세요'); 
        }
        else if(state.주소1===''){

            comfirmModalOpen( '주소를 검색 해주세요'); 
        }
        else if(state.주소2===''){
   
            comfirmModalOpen( '나머지 주소를 입력 해주세요'); 
        }
        else if( cnt < 3 ){ //이용약관동의 필수항목
 
            comfirmModalOpen('이용약관동의 필수항목 3개를 선택해야합니다.'); 
        }
        else{
