...
export default function QuickMenuComponent({product}){
    ...
    const getLocalStorage=()=>{
        ...
        if(localStorage.getItem( product.key)!==null){
            let viewData = JSON.parse(localStorage.getItem());
            setviewData(viewData);
        }
    }
    const viewProduct = state.viewData.map((item, idx)=>{
        return (
              <li key={idx}>{item.이미지}</li>
                  ...
                  {  
                        ...
                        <div className="view-box">
                            <ul className='wrap-box'>
                                <li key={1}><img src="'./images/intro/004ee48d-c745-4c58-b37f-f67a8f23b01f.jpg'" alt="" /></li>
                                {
