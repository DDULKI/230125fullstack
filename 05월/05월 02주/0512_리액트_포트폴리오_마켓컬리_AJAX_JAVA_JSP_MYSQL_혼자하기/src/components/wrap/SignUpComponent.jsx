    ...
    const onSubmitSignupEvent=(e)=>{
        ...
        if(state.아이디===''){
            ...
            const formData = {
                ...
                "name" : state.이름,
                "email" : state.이메일,
                "hp" : state.휴대폰.replace(regExpHp, '$1-$2-$3'),
                "addr" : `${state.주소1} ${state.주소2}`,
                "gender" : state.성별 ,
                "birth" : `${state.생년}-${state.생월}-${state.생일}`,
                "chooga" : `${state.추가입력사항} ${state.추천인아이디} ${state.참여이벤트명}`,
                "service" : 약관동의
            }
           ...
           console.log(formData);
            ...
            // http://192.168.40.101:8080/jsp/0512_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp
            // http://127.0.0.1:8080/jsp/0512_MVC_모델링_프로젝트/쇼핑몰고객관리/kurly_user_signup_action_test_parameter.jsp
            // localhost:8080/jsp/0512_MVC_모델링_프로젝트/쇼핑몰고객관리/kurly_user_signup_action_test_useBean.jsp
            $.ajax({
                // url:'http://127.0.0.1:8080/jsp/0512_MVC_모델링_프로젝트/쇼핑몰고객관리/kurly_user_signup_action.jsp',
                url:'http://127.0.0.1:8080/jsp/0512_MVC_모델링_프로젝트/쇼핑몰고객관리/kurly_user_signup_action.jsp',
                type: 'post',
                data: formData,
                success(res){
