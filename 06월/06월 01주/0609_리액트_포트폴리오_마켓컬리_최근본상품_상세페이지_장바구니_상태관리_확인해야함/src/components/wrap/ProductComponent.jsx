...
export default function ProductComponent ({cartCountNumber}) {
    ...
    const [state, setState] = React.useState({
        PRODUCT_KEY: 'MJ_KURLY_CART_PRODUCT',
        상품: {},
    });
    const [isCart, setIsCart]  = React.useState(false);
    const [isCartOk, setICartOk]  = React.useState(false);


    const {상품, PRODUCT_KEY} = state; // 비구조화
    ...
    const onClickCart=(e)=>{
        ...
        // 수량 총금액 계산값이 없다면
        if(isCart===false){ // 수량증감 변화가 없는상태 즉 수령증감 버튼 클릭안한상태 
            setState({
                ...state,
                상품 : {
                    ...state.상품,
    ...
    React.useEffect(()=>{
        ...
        if(isCartOk===true){ //장바구니를 클릭하면 실행 
            setIsCart(false); // 다음에 다시 클릭할 경우 초기화 버튼 클릭 안한 상태로 다시 유지
