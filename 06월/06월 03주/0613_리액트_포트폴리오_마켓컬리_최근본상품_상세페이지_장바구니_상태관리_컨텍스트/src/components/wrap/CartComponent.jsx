...
export default function CartComponent({openPopupDaumPostApi, addr, confirmModalOpen}) {
    ...
    const onClickSlideUpDown=(e, n)=>{
        ...
        if(isUp[n]===true){
            ddRef.current[n].style.display = '60px';
            isUp[n]=false;
        }
        else {
            ddRef.current[n].style.display = 'block';
            isUp[n]=true;
        }
        setIsUp([...isUp]);
    }
    ...
    const res3 = cart.map((item, idx)=>{  
        if(item.보관방법==='상온'){
            return(                                   
                <dd key={idx} ref={(el)=>(ddRef.current[2]=el)}>
                    <ul>
                        <li>
                            <input onChange={onChangeCheck} checked={check.includes(item.제품코드)} type="checkbox" name='chk' id='chk1' value={item.제품코드} />  
                        </li>
                        ...
                        <div className="content">
                            <div className="left">
                                <ul>
                                    <li> {/*  위쪽 전체선택 삭제 */}
                                        <span>
                                            <button onClick={onClickCheckAll} /* 체크박스일때는 이렇게 사용하면 댐 checked={check.length===cart.length?true:false} */ className={`select-all-btn${isCheckAll?' on':''}`}>
                                                전체선택
                                                <strong>
                                                    (
                                            ...
                                            (
                                                <>
                                                    ...
                                                    {/* 장바구니 상품 목록 시작  */}
                                                    {
                                                        ...
                                                        arr1.length >=1 && (
                                                            <div className='acodian' /* ref={(el)=>(ddRef.current)} */>
                                                                <dl>
                                                                    <dt>
                                                                        <div><img src="./images/cart/icon_01.svg" alt="" /><h3>냉장 상품</h3></div> 
                                                                        <div><button className='on' onClick={(e)=>onClickSlideUpDown(e,0)}><img className={!isUp[0]?'on':''} src="./images/cart/icon_arrow_up.svg" alt="" /></button></div>
                                                                    </dt>
                                                                { res1 }
                                                                </dl>
                                                            </div>
