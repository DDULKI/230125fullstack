...
export default function WrapComponent() {
    ...
    // 자식컴포넌트에서 클릭 닫는다. => 1년동안 설정 다시 안보기 
    ...
    React.useEffect(()=>{
        ...
        if(localStorage.getItem('MAINMODALONEYEARCLOSE')!==null){
            ...
            if( toDay >= result.expires ){ // 만료일 지나면 모달을 연다 
                setMainModal(true);
    ...
    const [modal, setModal]  =  React.useState({
        ...
        confirmOkCancelMsg: '확인 취소 컨펌모달창입니다. ',
        isConfirmOkCancelModal: false,
        isConfirmOkCancelModalResult:''

       
    });
    const {confirmMsg,isConfirmModal,confirmOkCancelMsg,isConfirmOkCancelModalResult} = modal;

    // 삭제 확인, 취소 컨펌 모달창 열기 메서드
    const ConfirmOkCancelModalOpen=(msg)=>{
        setModal({
            ...modal,
            confirmOkCancelMsg: msg,
            isConfirmOkCancelModal: true
        })
    }

    // 삭제 확인, 취소 컨펌 모달창 닫기 메서드 
    const ConfirmOkCancelModalClose=(value)=>{
        setModal({
            ...modal,
            isConfirmOkCancelModal: false,
            isConfirmOkCancelModalResult:value
        })
    }
    ...
    return (
        <div id='wrap'>
            <GlobalContext.Provider value={{ CARTPRODUCTKEY, VIEWPRODUCTKEY, TOPMODALKEY, ADDRESSKEY, isAdmin, mainModal, mainModalShowOut, mainModalOneYearShowOut, addressAuto,postCodeClose }}>
                <ConfirmContext.Provider value={{ confirmMsg, confirmModalOpen, confirmModalClose, isConfirmModal, ConfirmOkCancelModalOpen, confirmOkCancelMsg, ConfirmOkCancelModalClose, isConfirmOkCancelModalResult}}>
                    <CartContext.Provider value={{ cartCountNumber, cartCount }}>
                        <AddressApiContext.Provider value={{openPopupDaumPostApi,addr}}>
                                ...
                                <BrowserRouter basename={process.env.PUBLIC_URL}>
                                    <Routes>
                                        <Route path='/' element={<HeaderComponent setIsIntroFn={setIsIntroFn} />}>
                                            ...
                                            <Route path='/idSearchReset'  element={<SignInIdSearchResultComponent />} />
                            ...
                            {
                                modal.isConfirmOkCancelModal && <ConfirmOkCancel/>
                            }
