...
export default function SignUpComponent({회원가입, timer, timerCounterfn, mapAddressFn}) {
    ...
    const onChangeUserService=(e)=>{
        ...
        if(e.target.checked){ // 개별체크 선택항목 value 값을 이용약관동의 배열에 저장시킨다. 트루가 생략되어있다. 
           
            // 무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택) 선택(체크되면)이 되면 
            // SNS, 이메일 체크해야한다. 
            // 체크된다는건 이용약관동의 배열에 SNS와 이메일을 저장한다는것. 
            if(e.target.value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)' && state.이용약관동의.includes('SNS')===false && state.이용약관동의.includes('이메일')===false ){
                ...
                setState({
                    ...state, 
                    이용약관동의:[...state.이용약관동의, e.target.value, 'SNS'] // '이메일'은 있어요. 넣지 마세요.
                })
                ...
                setState({
                    ...state, 
                    이용약관동의:[...state.이용약관동의, e.target.value, '이메일'] // 'SNS'은 있어요. 넣지 마세요.
                })
            ...
            // 현재 SNS를 체크했는데 이메일이 체크가 되어있고, 그리고 무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택) 선택이 안되어있다면
                ...
                // 현재 체크해제한게 이메일이고 SNS 체크되어 있다면 이메일만 삭제 
    ...
    const onChangeGender=(e)=>{
        ...
        const onSubmitSignupEvent=(e)=>{ 
            e.preventDefault(); // 버튼에 새로고침버튼 지워주기 
            ...
            // }
            // $.ajax({
            //     url:'http://ddulki4994.dothome.co.kr/signup_db/insert.php',
            //     type: 'POST',
            //     data: formData,
            //     success(res){
            ...
            // 가입하기
            // 입력폼 화면의 필수 항목과 선택 입력항목, 그리고 중복확인, 인증등 항목들의 
            // 빠짐없는 항목을 체크하고 가용성있는 폼데이터를 전송한다.  
            // 1. 아이디 : 빈값이면 입력 요구 
            // 2. 아이디 중복확인 : 중복확인을 검사한다. isIdOk 
            // 3. 비밀번호 : 빈값이면 입력 요구 
            // 4. 비밀번호확인 : 두개비밀번호 비교 확인  isPw2 false이고 빈값이 아니면
            // 5. 이름: 빈값이면 입력요구
            // 6. 이메일: 빈값이면 입력요구
            // 7. 이메일 중복확인 : isEmailOk
            // 8. 휴대폰: 빈값이면 입력요구 
            // 9. 휴대폰: 인증번호 성공 여부확인 // 인증성공 추가 isHpOk true이면 성공
            // 10. 주소1, 주소2 : 빈값이면 입력요구 

            // 11. 성별 : 선택사항이므로 유효성에서 제외
            // 12. 생년월 : 선택사항이므로 유효성에서 제외
            // 13. 추가입력사항 : 선택사항이므로 유효성에서 제외

            // 14. 이용약관동의 : // 필수항목 3개 확인 추가 검증 if문 쓰기전에 마지막에 할 일. 
            //     가입하기 클릭하면 이용약관동의 배열값내용중 필수 항목을 카운트한다. 

            // 15. 1 ~ 14까지 이상없으면 전송

            // 선택자는 스테이트 
            // const result = state.이용약관동의.map((item)=>item.includes('필수')===true? 1:0); // 트루 없애도됨. 
            // console.log(result);
            // let sum=0; 
            // result.map((item)=>{
            //     sum+=item;
            ...
            // indexOf('필수') 찾으면 0 1 2 3... 글자 위치 인덱스 
            // 찾지 못하면 -1 
            ...
            // 8. 휴대폰  
            else if(state.휴대폰===false){
                setState({
                    ...state,
                    isConfirmModal:true,
                    confirmMsg: '휴대폰 번호를 입력해 주세요. '
                })
            }
            ...
            // 10. 
               ...
               //  모두 통과되면 전송 
                ...
                // 중간 숫자 3~4자리. 
                // 끝   숫자4자리 
                const regExpHp = /^(\d{3})(\d{3,4})(\d{4})$/g; // 010-7771-6035
                ...
                newFormData.append('user_app_input',    `${state.추가입력사항} ${state.추천인아이디} ${state.참여이벤트명}`);
                ...
                // Node.js 기반에서 리액트가 있는 위치 http://localhost:3002/ => http://ddulki4994.dothome.co.kr/signup_db/form_data_insert.php
                // 프로토콜: // 도메인: 포트번호/경로/index.html 
                // 브라우저가 보내는 주소(출처)와 받는 주소가 같은지 검사 정책 SOP RFC 6454 보안정책
                // => 출처가 같은 출처에서만 공유할 수 있다는 정책 규칙   
                axios({
                    url:'http://ddulki4994.dothome.co.kr/signup_db/form_data_insert.php', // 닷홈 호스팅 서버 : 서버사이드 스크립트 언어(JSP, PHP, ASP) => SQL => 데이터베이스 저장소 
                    method: 'POST',
                    data: newFormData
                }) 
                .then((res)=>{
